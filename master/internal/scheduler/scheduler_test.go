...
func setupCluster(
	...
	for _, handler := range tasks {
		...
		d.addAssignedTask(&AllocateRequest{
...
func assertSchedulerState(
	...
	for index, handler := range actual {
		...
		actualAssigned := rp.taskList.GetAssignments(handler)
		actualContainers := make(map[*agentState]int)
		if actualAssigned != nil {
			for _, assignment := range actualAssigned.Allocations {
...
func forceSchedule(rp *DefaultRP, handler *actor.Ref, agent *agentState) {
	...
	assigned := rp.taskList.GetAssignments(handler)
	if assigned == nil {
		assigned = &ResourcesAllocated{ID: req.ID}
	}
	...
	rp.taskList.SetAssignments(handler, assigned)
}
